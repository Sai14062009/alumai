...
import { useState, useEffect, useMemo, useCallback } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { RefreshCw, AlertTriangle, ShieldCheck, Wifi, X, Zap, Clock, AlertOctagon } from 'lucide-react';
import { getAllTicketsWithStats } from '../services/api';
import SeverityPill from '../components/SeverityPill';
...
const PLANT_SECTIONS = [
  {
    ...
    groups: [
      { label: 'Melters', cols: 4, mobileCols: 4, assets: ['Melter-1','Melter-2','Melter-3','Melter-4','Melter-5','Melter-6','Melter-7','Melter-8'] },
      { label: 'Holders', cols: 4, mobileCols: 4, assets: ['Holder-1','Holder-2','Holder-3','Holder-4','Holder-5','Holder-6','Holder-7','Holder-8'] },
      { label: 'Casting Pits', cols: 3, mobileCols: 3, assets: ['Casting Pit-1','Casting Pit-2','Casting Pit-3','Casting Pit-4','Casting Pit-5'] },
      { label: 'Support', cols: 1, mobileCols: 1, assets: ['Transfer Launder','Degassing Unit','Metal Treatment Station'] },
    ]
  },
  {
    id: 'plate', title: 'Plate', subtitle: 'Scalp → Treat → Finish',
    groups: [
      { label: 'Processing', cols: 1, mobileCols: 1, assets: ['Scalper','Reheat Furnace','Stretcher','Ager'] },
      { label: 'Finishing', cols: 1, mobileCols: 1, assets: ['Plate Saw','Ultrasonic Inspection'] },
    ]
  },
  {
    id: 'rolling', title: 'Rolling', subtitle: 'Hot → Cold Mill',
    groups: [
      { label: 'Hot Rolling', cols: 1, mobileCols: 1, assets: ['Hot Rolling Mill','Roughing Stand','Finishing Stand','Coil Box'] },
      { label: 'Cold Rolling', cols: 1, mobileCols: 1, assets: ['Cold Rolling Mill','Roll Grinding','Annealing Furnace'] },
    ]
  },
  {
    id: 'finishing', title: 'Sheet Finishing', subtitle: 'Level → Cut → Ship',
    groups: [
      { label: 'Processing', cols: 1, mobileCols: 1, assets: ['Tension Leveler','Slitter','Cut-to-Length Line'] },
      { label: 'QC & Shipping', cols: 1, mobileCols: 1, assets: ['Inspection Line','Packaging Line'] },
    ]
  },
];
...
function computeAssetStatuses(allTickets) {
  ...
  allTickets.forEach(ticket => {
    ...
    else { for (const p of ALL_PLANT_ASSETS) { if (assetName.toLowerCase().includes(p.toLowerCase()) || p.toLowerCase().includes(assetName.toLowerCase())) { matched = p; break; } } }
    if (!matched) return;
    statusMap[matched].tickets.push(ticket);
    const tStatus = ticket.status || '';
    const isResolved = tStatus.includes('Resolved') || tStatus === 'Resolved ✅';
...
export default function AssetMonitor() {
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto space-y-4 pb-12" style={{ overflow: 'hidden' }}>
        ...
        <div className="min-w-0">
          <h1 className="font-display font-extrabold text-xl sm:text-2xl md:text-3xl tracking-tight flex flex-wrap items-center gap-2" style={{ color: 'var(--text-primary)' }}>
            ...
            <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-full font-mono text-[8px] sm:text-[10px] font-bold uppercase tracking-widest"
              style={{ background: 'rgba(255,77,106,0.08)', color: '#FF4D6A', border: '1px solid rgba(255,77,106,0.12)' }}>
              <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full animate-pulse" style={{ background: '#FF4D6A' }} />
              LIVE
            </span>
          </h1>
          <p className="font-sans text-[11px] sm:text-xs md:text-sm mt-1 font-light" style={{ color: 'var(--text-muted)' }}>
            {stats.total} components · 4 sections
          </p>
        </div>
        <motion.button whileTap={{ scale: 0.95 }} onClick={fetchData}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
        {[
          ...
          <motion.div key={i} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
            className="glass gradient-border rounded-xl sm:rounded-2xl p-3 sm:p-4 flex items-center justify-between">
            <div>
      ...
      {/* Dot Matrix — 2x2 grid on mobile, single row on desktop */}
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
            ...
            <div key={section.id}>
              ...
              <div className="flex flex-wrap gap-[3px] sm:gap-[5px] justify-center">
                {section.groups.flatMap(g => g.assets).map((asset) => {
                  ...
                  const colors = { online: '#00E59B', fault: '#FF4D6A', pending: '#FFB800' };
                  return (
                    <div key={asset} onClick={() => setSelectedAsset(asset)}
                      className={`w-[10px] h-[10px] sm:w-[13px] sm:h-[13px] rounded-full cursor-pointer ${isFault ? 'animate-[dotPulse_2s_ease-in-out_infinite]' : ''}`}
                      style={{ background: colors[s] || colors.online, boxShadow: isFault ? '0 0 6px rgba(255,77,106,0.5)' : 'none' }}
                    />
                  );
                })}
              </div>
      ...
      {/* Plant Sections — stacked on mobile, 2 cols on tablet, 4 on desktop */}
        ...
        {PLANT_SECTIONS.map((section, si) => {
          ...
          return (
            <motion.div key={section.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + si * 0.08 }}
              ...
              <div className="px-3 py-2 sm:py-3 flex items-center justify-between" style={{ borderBottom: '1px solid var(--border)' }}>
                <div className="min-w-0">
              ...
              <div className="p-2 sm:p-3 space-y-2">
                {section.groups.map((group, gi) => (
                  <div key={gi}>
                    <p className="font-mono text-[7px] sm:text-[8px] uppercase tracking-[0.1em] mb-1.5 px-0.5" style={{ color: 'var(--text-muted)' }}>{group.label}</p>
                    <div className="grid gap-[3px]" style={{ gridTemplateColumns: `repeat(${group.mobileCols}, 1fr)` }}>
                      {group.assets.map((asset) => {
        ...
        {selectedAsset && selectedInfo && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center"
            ...
            <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }}
              ...
              className="glass rounded-t-[24px] sm:rounded-[24px] w-full sm:max-w-lg max-h-[85vh] overflow-hidden flex flex-col"
              ...
              <div className="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between" style={{ borderBottom: '1px solid var(--border)' }}>
                <div className="flex items-center gap-3 min-w-0">
                  <span className={`w-3 h-3 rounded-full flex-shrink-0 ${selectedInfo.status === 'fault' ? 'animate-pulse' : ''}`}
                    style={{ background: selectedInfo.status === 'fault' ? '#FF4D6A' : selectedInfo.status === 'pending' ? '#FFB800' : '#00E59B' }} />
                  <div className="min-w-0">
                    <h3 className="font-display font-bold text-[14px] sm:text-[16px] truncate" style={{ color: 'var(--text-primary)' }}>{selectedAsset}</h3>
                    <p className="font-mono text-[9px] sm:text-[10px] uppercase font-semibold" style={{
                      color: selectedInfo.status === 'fault' ? '#FF4D6A' : selectedInfo.status === 'pending' ? '#FFB800' : '#00E59B'
                    }}>{selectedInfo.status === 'fault' ? 'Active Fault' : selectedInfo.status === 'pending' ? 'Pending' : 'Online'}</p>
                  </div>
                </div>
                <motion.button whileTap={{ scale: 0.85 }} onClick={() => setSelectedAsset(null)}
                  className="w-8 h-8 rounded-xl flex items-center justify-center haptic flex-shrink-0"
                  style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
              ...
              <div className="px-4 sm:px-6 pt-4">
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <div key={i} className="rounded-xl p-2.5 sm:p-3 text-center" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
                      <p className="font-display font-extrabold text-lg sm:text-xl" style={{ color: s.c }}>{s.v}</p>
                      <p className="font-mono text-[7px] sm:text-[8px] uppercase tracking-[0.1em]" style={{ color: 'var(--text-muted)' }}>{s.l}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-3">
                ...
                {selectedInfo.tickets.filter(t => !t.status?.includes('Resolved') && (t.is_classified || t.status === 'Escalated')).length > 0 && (
                  <div>
                    <p className="font-mono text-[8px] uppercase tracking-[0.15em] font-bold mb-2" style={{ color: '#FF4D6A' }}>Active Issues</p>
                    <div className="space-y-2">
                      {selectedInfo.tickets.filter(t => !t.status?.includes('Resolved') && (t.is_classified || t.status === 'Escalated')).map((t, i) => (
                        <div key={i} className="rounded-xl p-3" style={{ background: 'rgba(255,77,106,0.04)', border: '1px solid rgba(255,77,106,0.1)' }}>
                          <div className="flex items-center justify-between mb-1.5">
                            <div className="flex items-center gap-2">
                              <span className="font-mono text-[10px] font-bold" style={{ color: '#00E59B' }}>#{t.id}</span>
                              {(t.severity || t.priority) && <SeverityPill level={t.severity || t.priority} />}
                            ...
                            <span className="font-mono text-[8px] uppercase font-semibold px-1.5 py-0.5 rounded"
                              style={{ color: '#FF4D6A', background: 'rgba(255,77,106,0.1)' }}>{t.status}</span>
                          </div>
                          <p className="font-sans text-[10px] sm:text-[11px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                ...
                {/* Awaiting RCA */}
                {selectedInfo.tickets.filter(t => !t.is_classified && !t.status?.includes('Resolved')).length > 0 && (
                  <div>
                    <p className="font-mono text-[8px] uppercase tracking-[0.15em] font-bold mb-2" style={{ color: '#B18CFF' }}>Awaiting RCA</p>
                    <div className="space-y-1.5">
                      {selectedInfo.tickets.filter(t => !t.is_classified && !t.status?.includes('Resolved')).map((t, i) => (
                        <div key={i} className="rounded-lg p-2.5 flex items-start gap-2" style={{ background: 'rgba(177,140,255,0.04)', border: '1px solid rgba(177,140,255,0.08)' }}>
                          <span className="font-mono text-[9px] font-bold flex-shrink-0" style={{ color: '#B18CFF' }}>#{t.id}</span>
                ...
                {selectedInfo.resolvedCount > 0 && (
                  <div>
                    <p className="font-mono text-[8px] uppercase tracking-[0.15em] font-bold mb-2" style={{ color: '#00E59B' }}>Resolved</p>
                    <div className="space-y-1">
                      {selectedInfo.tickets.filter(t => t.status?.includes('Resolved')).map((t, i) => (
                        <div key={i} className="rounded-lg p-2 flex items-center gap-2" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
                          <span className="font-mono text-[9px] font-bold" style={{ color: '#00E59B' }}>#{t.id}</span>
                          <span className="font-sans text-[9px] truncate flex-1" style={{ color: 'var(--text-muted)' }}>{(t.description || '').substring(0, 50)}...</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {selectedInfo.tickets.length === 0 && (
                  <div className="text-center py-8">
                    <ShieldCheck className="w-10 h-10 mx-auto mb-3" style={{ color: '#00E59B', opacity: 0.3 }} />
                    <p className="font-display font-bold text-sm" style={{ color: 'var(--text-primary)' }}>All Clear</p>
                    <p className="font-sans text-[11px]" style={{ color: 'var(--text-muted)' }}>No tickets for {selectedAsset}</p>
                  </div>
                )}
              </div>

              {/* Footer */}
              <div className="px-4 sm:px-6 py-3 flex items-center justify-between" style={{ borderTop: '1px solid var(--border)' }}>
                <p className="font-mono text-[9px]" style={{ color: 'var(--text-muted)' }}>{selectedInfo.tickets.length} tickets</p>
                <button onClick={() => setSelectedAsset(null)}
                  className="haptic px-4 py-2 rounded-xl font-display text-[11px] font-bold"
                  style={{ background: 'var(--bg-surface)', color: 'var(--text-secondary)', border: '1px solid var(--border)' }}>Close</button>
              </div>
            </motion.div>
          </div>
        )}
