...
const PLANT_SECTIONS = [
  {
    id: 'ingot', title: 'Ingot Casting', subtitle: 'Melting → Holding → Casting',
    groups: [
      { label: 'Melters', cols: 4, assets: ['Melter-1','Melter-2','Melter-3','Melter-4','Melter-5','Melter-6','Melter-7','Melter-8'] },
      { label: 'Holders', cols: 4, assets: ['Holder-1','Holder-2','Holder-3','Holder-4','Holder-5','Holder-6','Holder-7','Holder-8'] },
      { label: 'Casting Pits', cols: 3, assets: ['Casting Pit-1','Casting Pit-2','Casting Pit-3','Casting Pit-4','Casting Pit-5'] },
  ...
  {
    id: 'plate', title: 'Plate', subtitle: 'Scalping → Heat Treat → Finishing',
    groups: [
      { label: 'Processing Line', cols: 1, assets: ['Scalper','Reheat Furnace','Stretcher','Ager'] },
      { label: 'Finishing', cols: 1, assets: ['Plate Saw','Ultrasonic Inspection'] },
    ]
  ...
  {
    id: 'rolling', title: 'Rolling', subtitle: 'Hot Mill → Cold Mill',
    groups: [
      { label: 'Hot Rolling', cols: 1, assets: ['Hot Rolling Mill','Roughing Stand','Finishing Stand','Coil Box'] },
      { label: 'Cold Rolling', cols: 1, assets: ['Cold Rolling Mill','Roll Grinding','Annealing Furnace'] },
    ]
  ...
  {
    id: 'finishing', title: 'Sheet Finishing', subtitle: 'Leveling → Cutting → Shipping',
    groups: [
      { label: 'Processing', cols: 1, assets: ['Tension Leveler','Slitter','Cut-to-Length Line'] },
      { label: 'Quality & Shipping', cols: 1, assets: ['Inspection Line','Packaging Line'] },
    ]
...
function computeAssetStatuses(allTickets) {
  ...
  allTickets.forEach(ticket => {
    const assetName = ticket.asset_name || ticket.asset || '';
    let matched = null;
    if (statusMap[assetName] !== undefined) { matched = assetName; }
    else {
      for (const plantAsset of ALL_PLANT_ASSETS) {
        if (assetName.toLowerCase().includes(plantAsset.toLowerCase()) || plantAsset.toLowerCase().includes(assetName.toLowerCase())) { matched = plantAsset; break; }
      }
    }
    if (!matched) return;
    ...
    if (isResolved) { statusMap[matched].resolvedCount++; }
    else if (isEscalated) { statusMap[matched].activeCount++; statusMap[matched].status = 'fault'; }
    else if (isClassified) { statusMap[matched].activeCount++; if (statusMap[matched].status === 'online') statusMap[matched].status = 'pending'; }
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto space-y-4 md:space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2 mb-2">
        <div>
          <h1 className="font-display font-extrabold text-2xl md:text-3xl tracking-tight flex items-center gap-3" style={{ color: 'var(--text-primary)' }}>
            DPW Plant Monitor
            <span className="flex items-center gap-1.5 px-2 md:px-3 py-1 rounded-full font-mono text-[9px] md:text-[10px] font-bold uppercase tracking-widest"
              style={{ background: 'rgba(255,77,106,0.08)', color: '#FF4D6A', border: '1px solid rgba(255,77,106,0.12)' }}>
              <span className="w-2 h-2 rounded-full animate-pulse" style={{ background: '#FF4D6A', boxShadow: '0 0 8px rgba(255,77,106,0.4)' }} />
              LIVE
            </span>
          </h1>
          <p className="font-sans text-xs md:text-sm mt-1 font-light" style={{ color: 'var(--text-muted)' }}>
            DPW Aluminum — {stats.total} components across 4 sections
          </p>
        </div>
        <motion.button whileTap={{ scale: 0.95 }} onClick={fetchData}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <motion.div key={i} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
            className="glass gradient-border rounded-2xl md:rounded-3xl p-3 md:p-5 flex items-center justify-between relative overflow-hidden">
            <div className="relative z-10">
              <p className="font-sans text-[10px] md:text-[12px] font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>{s.label}</p>
              <p className="font-display text-xl md:text-3xl font-extrabold" style={{ color: s.color }}>{s.value}</p>
            </div>
            <div className="w-9 h-9 md:w-11 md:h-11 rounded-xl md:rounded-2xl flex items-center justify-center relative z-10"
              style={{ background: `${s.color}10`, border: `1px solid ${s.color}15` }}>
      ...
      {/* Dot Matrix */}
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
        className="glass gradient-border rounded-2xl md:rounded-3xl p-3 md:p-5">
        <div className="flex items-center justify-between mb-3 md:mb-4">
          <p className="font-mono text-[9px] md:text-[10px] uppercase tracking-[0.2em] font-semibold" style={{ color: 'var(--text-muted)' }}>Status Matrix</p>
          <div className="flex items-center gap-3 md:gap-5">
            {[{ l: 'Online', c: '#00E59B' }, { l: 'Fault', c: '#FF4D6A' }, { l: 'Pending', c: '#FFB800' }].map((x, i) => (
              <div key={i} className="flex items-center gap-1">
                <span className="w-2 h-2 md:w-2.5 md:h-2.5 rounded-full" style={{ background: x.c }} />
                <span className="font-mono text-[8px] md:text-[9px]" style={{ color: 'var(--text-muted)' }}>{x.l}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {PLANT_SECTIONS.map((section) => (
            <div key={section.id}>
              <p className="font-mono text-[8px] md:text-[9px] font-semibold uppercase tracking-[0.15em] mb-2 md:mb-3 text-center" style={{ color: 'var(--text-muted)' }}>{section.title}</p>
              <div className="flex flex-wrap gap-[4px] md:gap-[5px] justify-center">
                {section.groups.flatMap(g => g.assets).map((asset) => {
                  const s = statusMap[asset]?.status || 'online';
                  const isFault = s === 'fault';
                  const colors = { online: '#00E59B', fault: '#FF4D6A', pending: '#FFB800' };
                  return (
                    <motion.div key={asset} whileHover={{ scale: 1.5 }}
                      onClick={() => setSelectedAsset(asset)}
                      className={`w-[12px] h-[12px] md:w-[14px] md:h-[14px] rounded-full cursor-pointer ${isFault ? 'animate-[dotPulse_2s_ease-in-out_infinite]' : ''}`}
                      style={{ background: colors[s] || colors.online, boxShadow: isFault ? '0 0 8px rgba(255,77,106,0.5)' : s === 'pending' ? '0 0 6px rgba(255,184,0,0.3)' : `0 0 4px ${colors[s]}30` }}
                      title={`${asset} — ${s}`}
                    />
                  );
                })}
        ...
        {PLANT_SECTIONS.map((section, si) => {
          ...
          return (
            <motion.div key={section.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + si * 0.08 }}
              className="glass gradient-border rounded-2xl md:rounded-3xl overflow-hidden">
              <div className="px-3 md:px-4 py-2.5 md:py-3 flex items-center justify-between" style={{ borderBottom: '1px solid var(--border)' }}>
                <div>
                  <h3 className="font-display font-bold text-[12px] md:text-[13px]" style={{ color: 'var(--text-primary)' }}>{section.title}</h3>
                  <p className="font-sans text-[9px] md:text-[10px]" style={{ color: 'var(--text-muted)' }}>{section.subtitle}</p>
                </div>
                <span className="font-mono text-[11px] md:text-[12px] font-bold" style={{ color: healthColor }}>{health}%</span>
              </div>
              <div className="p-2 md:p-3 space-y-2 md:space-y-3">
                {section.groups.map((group, gi) => (
                  <div key={gi}>
                    <p className="font-mono text-[7px] md:text-[8px] uppercase tracking-[0.15em] mb-1.5 md:mb-2 px-1" style={{ color: 'var(--text-muted)' }}>{group.label}</p>
                    <div className="grid gap-[3px] md:gap-[4px]" style={{ gridTemplateColumns: `repeat(${group.cols}, 1fr)` }}>
                      {group.assets.map((asset) => {
                        const info = statusMap[asset] || { status: 'online', tickets: [], activeCount: 0 };
                        const short = SHORT_NAMES[asset] || asset;
                        const s = info.status;
                        ...
                        const dotColor = isFault ? '#FF4D6A' : isPending ? '#FFB800' : '#00E59B';
                        return (
                          <motion.div key={asset} whileTap={{ scale: 0.93 }} onClick={() => setSelectedAsset(asset)}
                            className={`rounded-lg px-1.5 md:px-2 py-1 md:py-1.5 cursor-pointer flex items-center gap-1 md:gap-1.5 transition-all ${isFault ? 'animate-[unitGlow_2s_ease-in-out_infinite]' : ''}`}
                            style={{ background: bg, border: `${isSelected ? '2' : '1'}px solid ${border}` }}>
                            <span className={`w-[5px] h-[5px] md:w-[6px] md:h-[6px] rounded-full flex-shrink-0 ${isFault ? 'animate-pulse' : ''}`} style={{ background: dotColor }} />
                            <span className="font-mono text-[8px] md:text-[9px] font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{short}</span>
                            {info.activeCount > 0 && (
                              <span className="font-mono text-[6px] md:text-[7px] font-bold ml-auto px-1 py-0.5 rounded-full"
                                style={{ background: isFault ? 'rgba(255,77,106,0.15)' : 'rgba(255,184,0,0.1)', color: dotColor }}>{info.activeCount}</span>
                            )}
                          </motion.div>
                        );
                      })}
      ...
      <AnimatePresence>
        {selectedAsset && selectedInfo && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-2 md:p-4"
            style={{ background: 'rgba(4,6,12,0.88)', backdropFilter: 'blur(16px)' }}
            ...
            <motion.div initial={{ y: 50, opacity: 0, scale: 0.92 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 30, opacity: 0, scale: 0.95 }}
              ...
              className="glass rounded-[20px] md:rounded-[28px] w-full max-w-2xl max-h-[95vh] md:max-h-[85vh] overflow-hidden relative flex flex-col mx-1 md:mx-0"
              style={{ boxShadow: `0 0 80px ${selectedInfo.status === 'fault' ? 'rgba(255,77,106,0.08)' : 'rgba(0,229,155,0.08)'}, 0 24px 64px rgba(0,0,0,0.5)` }}>

              <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.8 }}
                className="absolute top-0 left-0 right-0 h-[2px]"
                style={{ background: `linear-gradient(90deg, transparent, ${selectedInfo.status === 'fault' ? '#FF4D6A' : selectedInfo.status === 'pending' ? '#FFB800' : '#00E59B'}, transparent)`, transformOrigin: 'center', opacity: 0.6 }} />

              {/* Header */}
              <div className="p-4 md:p-7 flex items-center justify-between" style={{ borderBottom: '1px solid var(--border)' }}>
                <div className="flex items-center gap-3 md:gap-4">
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex items-center justify-center relative"
                    style={{ background: selectedInfo.status === 'fault' ? 'rgba(255,77,106,0.12)' : selectedInfo.status === 'pending' ? 'rgba(255,184,0,0.12)' : 'rgba(0,229,155,0.12)',
                      border: `1px solid ${selectedInfo.status === 'fault' ? 'rgba(255,77,106,0.18)' : selectedInfo.status === 'pending' ? 'rgba(255,184,0,0.18)' : 'rgba(0,229,155,0.18)'}` }}>
                    {selectedInfo.status === 'fault' ? <AlertOctagon className="w-5 h-5 md:w-6 md:h-6" style={{ color: '#FF4D6A' }} /> :
                     selectedInfo.status === 'pending' ? <Clock className="w-5 h-5 md:w-6 md:h-6" style={{ color: '#FFB800' }} /> :
                     <ShieldCheck className="w-5 h-5 md:w-6 md:h-6" style={{ color: '#00E59B' }} />}
                    {selectedInfo.status === 'fault' && <span className="absolute -top-1 -right-1 w-3 h-3 md:w-4 md:h-4 rounded-full animate-pulse" style={{ background: '#FF4D6A', boxShadow: '0 0 8px rgba(255,77,106,0.5)' }} />}
                  </div>
                  <div>
                    <h2 className="font-display font-bold text-base md:text-xl" style={{ color: 'var(--text-primary)' }}>{selectedAsset}</h2>
                    <p className="font-mono text-[9px] md:text-[11px] mt-0.5 uppercase font-semibold tracking-wider" style={{
                      color: selectedInfo.status === 'fault' ? '#FF4D6A' : selectedInfo.status === 'pending' ? '#FFB800' : '#00E59B'
                    }}>{selectedInfo.status === 'fault' ? 'Active Fault' : selectedInfo.status === 'pending' ? 'Pending Review' : 'Online'}</p>
                  </div>
                </div>
                <motion.button whileTap={{ scale: 0.85 }} onClick={() => setSelectedAsset(null)}
                  className="w-8 h-8 md:w-10 md:h-10 rounded-xl md:rounded-2xl flex items-center justify-center haptic"
                  style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
              ...
              <div className="px-4 md:px-7 pt-4 md:pt-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
                  {[
                    { l: 'Total Tickets', v: selectedInfo.tickets.length, c: '#00D4FF' },
                    { l: 'Active', v: selectedInfo.activeCount, c: '#FF4D6A' },
                    { l: 'Resolved', v: selectedInfo.resolvedCount, c: '#00E59B' },
                    { l: 'Unclassified', v: selectedInfo.tickets.filter(t => !t.is_classified && !t.status?.includes('Resolved')).length, c: '#B18CFF' },
                  ].map((s, i) => (
                    <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.05 }}
                      className="rounded-xl md:rounded-2xl p-3 md:p-4 text-center" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
                      <p className="font-display font-extrabold text-lg md:text-2xl" style={{ color: s.c }}>{s.v}</p>
                      <p className="font-mono text-[7px] md:text-[8px] uppercase tracking-[0.15em] mt-1" style={{ color: 'var(--text-muted)' }}>{s.l}</p>
                    </motion.div>
                  ))}
                </div>
                {selectedInfo.worstPriority && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
                    ...
                    <span className="font-mono text-[8px] md:text-[9px] uppercase tracking-[0.15em]" style={{ color: 'var(--text-muted)' }}>Highest Priority:</span>
              ...
              {/* Ticket List */}
              <div className="flex-1 overflow-y-auto px-4 md:px-7 py-4 md:py-5 space-y-4">
                {/* Active */}
                {selectedInfo.tickets.filter(t => !t.status?.includes('Resolved') && (t.is_classified || t.status === 'Escalated')).length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-2 md:mb-3">
                      <AlertOctagon className="w-3.5 h-3.5" style={{ color: '#FF4D6A' }} />
                      <p className="font-mono text-[8px] md:text-[9px] uppercase tracking-[0.2em] font-bold" style={{ color: '#FF4D6A' }}>Active Issues</p>
                    </div>
                    <div className="space-y-2">
                      {selectedInfo.tickets.filter(t => !t.status?.includes('Resolved') && (t.is_classified || t.status === 'Escalated')).map((t, i) => (
                        <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.06 }}
                          className="rounded-xl md:rounded-2xl p-3 md:p-4" style={{ background: 'rgba(255,77,106,0.04)', border: '1px solid rgba(255,77,106,0.1)' }}>
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <span className="font-mono text-[10px] md:text-[11px] font-bold" style={{ color: '#00E59B' }}>#{t.id}</span>
                              {(t.severity || t.priority) && <SeverityPill level={t.severity || t.priority} />}
                            </div>
                            <span className="font-mono text-[8px] md:text-[9px] uppercase font-semibold px-1.5 md:px-2 py-0.5 rounded-lg"
                              style={{ color: t.status === 'Escalated' ? '#FF4D6A' : '#FFB800', background: t.status === 'Escalated' ? 'rgba(255,77,106,0.1)' : 'rgba(255,184,0,0.08)' }}>{t.status}</span>
                          </div>
                          <p className="font-sans text-[11px] md:text-[12px] leading-relaxed mb-2 md:mb-3" style={{ color: 'var(--text-secondary)' }}>{t.description}</p>
                          <div className="flex flex-wrap items-center gap-2 md:gap-4 pt-2" style={{ borderTop: '1px solid var(--border)' }}>
                            <span className="font-sans text-[9px] md:text-[10px]" style={{ color: 'var(--text-muted)' }}>{t.reported_by}</span>
                            <span className="font-mono text-[9px] md:text-[10px]" style={{ color: 'var(--text-muted)' }}>{t.created_at}</span>
                            <span className="font-sans text-[9px] md:text-[10px] capitalize px-1.5 py-0.5 rounded"
                              style={{ background: 'var(--bg-surface)', color: 'var(--text-muted)', border: '1px solid var(--border)' }}>{t.source}</span>
                          </div>
                        </motion.div>
                      ))}
                    </div>
                ...
                {/* Unclassified */}
                {selectedInfo.tickets.filter(t => !t.is_classified && !t.status?.includes('Resolved') && t.status !== 'Escalated').length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-2 md:mb-3">
                      <Clock className="w-3.5 h-3.5" style={{ color: '#B18CFF' }} />
                      <p className="font-mono text-[8px] md:text-[9px] uppercase tracking-[0.2em] font-bold" style={{ color: '#B18CFF' }}>Awaiting RCA</p>
                    </div>
                    <div className="space-y-2">
                      {selectedInfo.tickets.filter(t => !t.is_classified && !t.status?.includes('Resolved') && t.status !== 'Escalated').map((t, i) => (
                        <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + i * 0.04 }}
                          className="rounded-xl p-2.5 md:p-3 flex items-start gap-2 md:gap-3" style={{ background: 'rgba(177,140,255,0.04)', border: '1px solid rgba(177,140,255,0.08)' }}>
                          <span className="font-mono text-[9px] md:text-[10px] font-bold mt-0.5 flex-shrink-0" style={{ color: '#B18CFF' }}>#{t.id}</span>
                          <div className="flex-1 min-w-0">
                            <p className="font-sans text-[10px] md:text-[11px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                              {(t.description || '').substring(0, 120)}{(t.description || '').length > 120 ? '...' : ''}
                            </p>
                            <div className="flex flex-wrap items-center gap-2 md:gap-3 mt-1.5">
                              <span className="font-mono text-[8px] md:text-[9px]" style={{ color: 'var(--text-muted)' }}>{t.reported_by}</span>
                              {(t.severity || t.priority) && <SeverityPill level={t.severity || t.priority} />}
                            </div>
                          </div>
                        </motion.div>
                ...
                {selectedInfo.resolvedCount > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-2 md:mb-3">
                      <ShieldCheck className="w-3.5 h-3.5" style={{ color: '#00E59B' }} />
                      <p className="font-mono text-[8px] md:text-[9px] uppercase tracking-[0.2em] font-bold" style={{ color: '#00E59B' }}>Resolved</p>
                    </div>
                    <div className="space-y-1.5">
                      {selectedInfo.tickets.filter(t => t.status?.includes('Resolved')).map((t, i) => (
                        <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 + i * 0.03 }}
                          className="rounded-xl p-2.5 md:p-3 flex items-center gap-2 md:gap-3" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
                          <span className="font-mono text-[9px] md:text-[10px] font-bold" style={{ color: '#00E59B' }}>#{t.id}</span>
                          <span className="font-sans text-[9px] md:text-[10px] truncate flex-1" style={{ color: 'var(--text-muted)' }}>{(t.description || '').substring(0, 60)}...</span>
                          <span className="font-mono text-[7px] md:text-[8px] uppercase px-1.5 py-0.5 rounded hidden sm:inline" style={{ color: '#00E59B', background: 'rgba(0,229,155,0.06)' }}>Resolved</span>
                        </motion.div>
                      ))}
                    </div>
                  </div>
                ...
                {selectedInfo.tickets.length === 0 && (
                  <div className="text-center py-8 md:py-12">
                    <ShieldCheck className="w-12 h-12 md:w-14 md:h-14 mx-auto mb-4" style={{ color: '#00E59B', opacity: 0.3 }} />
                    <h3 className="font-display font-bold text-base md:text-lg mb-1" style={{ color: 'var(--text-primary)' }}>All Systems Clear</h3>
                    <p className="font-sans text-[12px] md:text-[13px]" style={{ color: 'var(--text-muted)' }}>No tickets for {selectedAsset}</p>
                  </div>
                )}
              </div>

              {/* Footer */}
              <div className="px-4 md:px-7 py-3 md:py-4 flex items-center justify-between" style={{ borderTop: '1px solid var(--border)' }}>
                <p className="font-mono text-[9px] md:text-[10px]" style={{ color: 'var(--text-muted)' }}>{selectedInfo.tickets.length} total tickets</p>
                <motion.button whileTap={{ scale: 0.95 }} onClick={() => setSelectedAsset(null)}
                  className="haptic px-4 md:px-5 py-2 md:py-2.5 rounded-xl font-display text-[11px] md:text-[12px] font-bold"
                  style={{ background: 'var(--bg-surface)', color: 'var(--text-secondary)', border: '1px solid var(--border)' }}>Close</motion.button>
              </div>
            </motion.div>
          </div>
        )}
