...
import { useState, useEffect, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Send, CheckCircle2, Sparkles, RefreshCw, ChevronLeft } from 'lucide-react';
import ChatBubble from '../components/ChatBubble';
import SeverityPill from '../components/SeverityPill';
import { getEscalatedTickets, sendMessage, finalizeRCA } from '../services/api';
...
export default function Escalation() {
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      if (r.status === 'success') {
        confetti(); setToast(`Ticket #${activeTicket.id} Resolved`); setTimeout(()=>setToast(null),3000);
        const nt = tickets.filter(t=>t.id!==activeTicket.id); setTickets(nt);
        if (nt.length>0) selectTicket(nt[0]); else { setActiveTicket(null); setMessages([]); setShowChat(false); }
      } else setMessages(p=>[...p,{role:'ai',content:`Issue: ${r.message||"Cannot finalize."}`}]);
    } catch(e) { setMessages(p=>[...p,{role:'ai',content:`Error: ${e.message||"Failed."}`}]); }
  };
  ...
  return (
    <div className="w-full h-[calc(100vh-96px)] md:h-[calc(100vh-96px)] flex flex-col lg:flex-row gap-3 md:gap-4">
      <AnimatePresence>
        {toast && (
          <motion.div initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:0.9}}
            className="fixed top-20 md:top-6 right-4 md:right-6 z-50 glass rounded-2xl px-4 md:px-5 py-3 md:py-3.5 flex items-center gap-2.5 glow-ring"
            style={{borderLeft:'3px solid #00E59B'}}>
            <CheckCircle2 className="w-4 h-4" style={{color:'#00E59B'}} />
            <span className="font-sans text-[12px] md:text-[13px] font-semibold" style={{color:'var(--text-primary)'}}>{toast}</span>
          </motion.div>
        )}
      </AnimatePresence>

      {/* Left — Ticket list (hidden on mobile when chat is open) */}
      <div className={`${showChat ? 'hidden lg:flex' : 'flex'} w-full lg:w-[320px] flex-shrink-0 glass rounded-2xl md:rounded-3xl overflow-hidden flex-col`}>
        <div className="p-4 md:p-5 flex justify-between items-center" style={{borderBottom:'1px solid var(--border)'}}>
          <div>
            <h2 className="font-display font-bold text-[14px] md:text-[15px]" style={{color:'var(--text-primary)'}}>Escalated</h2>
            <p className="font-sans text-[10px] md:text-[11px] mt-0.5 font-light" style={{color:'var(--text-muted)'}}>Awaiting human review</p>
          </div>
          <motion.button whileTap={{scale:0.9}} onClick={fetchEscalated}
            className="haptic w-8 h-8 md:w-9 md:h-9 rounded-xl flex items-center justify-center" style={{background:'var(--bg-surface)',border:'1px solid var(--border)'}}>
            <RefreshCw className={`w-3.5 h-3.5 ${loading?'animate-spin':''}`} style={{color:'var(--text-muted)'}} />
          </motion.button>
        </div>
          ...
          {loading ? (
            ...
            <AnimatePresence>
              {tickets.map(t => (
                <motion.div key={t.id} layout initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,x:-20}}
                  onClick={()=>selectTicket(t)}
                  className="p-3 md:p-4 rounded-xl md:rounded-2xl cursor-pointer transition-all duration-200 group"
                  style={{
                    background: activeTicket?.id===t.id ? 'rgba(0,229,155,0.04)' : 'transparent',
                    border: `1px solid ${activeTicket?.id===t.id ? 'rgba(0,229,155,0.15)' : 'var(--border)'}`,
                    borderLeft: activeTicket?.id===t.id ? '3px solid #00E59B' : '1px solid var(--border)',
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <span className="font-mono text-[11px] md:text-[12px] font-bold" style={{color:'#00E59B'}}>#{t.id}</span>
                    {t.severity && <SeverityPill level={t.severity} />}
                  </div>
                  <p className="font-sans text-[10px] md:text-[11px] line-clamp-2 mb-1.5 font-light" style={{color:'var(--text-secondary)'}}>{t.description||"No description"}</p>
                  <span className="font-mono text-[8px] md:text-[9px]" style={{color:'var(--text-muted)'}}>Asset: {t.asset||'Unknown'}</span>
                </motion.div>
              ))}
            </AnimatePresence>
          )}
      ...
      {/* Right — Chat (full screen on mobile when active) */}
      <div className={`${!showChat && activeTicket ? 'hidden lg:flex' : !showChat ? 'hidden lg:flex' : 'flex'} flex-1 glass rounded-2xl md:rounded-3xl flex-col relative overflow-hidden`}>
        {activeTicket ? (
          <>
            {/* Chat Header */}
            <div className="p-3 md:p-5 flex items-center gap-3 md:gap-3.5 flex-shrink-0" style={{borderBottom:'1px solid var(--border)'}}>
              {/* Back button on mobile */}
              <motion.button whileTap={{scale:0.9}} onClick={handleBack}
                className="lg:hidden haptic w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0"
                style={{background:'var(--bg-surface)',border:'1px solid var(--border)'}}>
                <ChevronLeft className="w-4 h-4" style={{color:'var(--text-muted)'}} />
              </motion.button>
              <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl md:rounded-2xl flex items-center justify-center" style={{background:'rgba(0,229,155,0.08)',border:'1px solid rgba(0,229,155,0.12)'}}>
                <Sparkles className="w-3.5 h-3.5 md:w-4 md:h-4" style={{color:'#00E59B'}} />
              </div>
              <div className="flex-1 min-w-0">
                <h2 className="font-display font-bold text-[12px] md:text-[14px] flex items-center gap-2" style={{color:'var(--text-primary)'}}>
                  AI Diagnostic Partner
                  <span className="w-2 h-2 rounded-full animate-pulse" style={{background:'#00E59B',boxShadow:'0 0 8px rgba(0,229,155,0.4)'}} />
                </h2>
                <p className="font-mono text-[9px] md:text-[10px] truncate" style={{color:'var(--text-muted)'}}>Ticket #{activeTicket.id} · {activeTicket.source}</p>
              </div>
            </div>
            ...
            <div className="p-3 md:p-4 flex-shrink-0" style={{borderTop:'1px solid var(--border)'}}>
              <form onSubmit={handleSend} className="relative flex items-center">
                <input type="text" value={inputValue} onChange={e=>setInputValue(e.target.value)}
                  ...
                  className="w-full py-3 md:py-3.5 pl-4 md:pl-5 pr-12 md:pr-14 rounded-xl md:rounded-2xl font-sans text-[12px] md:text-[13px] outline-none transition-all duration-200"
                  style={{background:'var(--bg-surface)',color:'var(--text-primary)',border:'1px solid var(--border)'}}
                  onFocus={e=>e.target.style.borderColor='rgba(0,229,155,0.25)'}
                  onBlur={e=>e.target.style.borderColor='var(--border)'} />
                <button type="submit" disabled={!inputValue.trim()||isTyping}
                  className="absolute right-2 w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl flex items-center justify-center disabled:opacity-30 transition-all haptic"
                  style={{background:'linear-gradient(135deg, #00E59B, #00D4FF)',color:'#04060C'}}>
            ...
            {/* Finalize button */}
            <motion.button whileHover={{scale:1.03}} whileTap={{scale:0.95}} onClick={handleFinalize}
              className="absolute bottom-16 md:bottom-20 right-3 md:right-6 px-3 md:px-5 py-2 md:py-2.5 rounded-xl md:rounded-2xl font-display text-[10px] md:text-[12px] font-bold flex items-center gap-1.5 md:gap-2 z-20"
              style={{background:'linear-gradient(135deg, #00E59B, #00D4FF)',color:'#04060C',boxShadow:'0 0 24px rgba(0,229,155,0.15)'}}>
              <CheckCircle2 className="w-3.5 h-3.5 md:w-4 md:h-4" /> FINALIZE RCA
            </motion.button>
          </>
        ) : (
          <div className="w-full h-full flex flex-col items-center justify-center" style={{color:'var(--text-muted)'}}>
            <Sparkles className="w-10 h-10 mb-4 opacity-15" />
            <p className="font-sans text-[12px] md:text-[13px] font-light">{tickets.length===0?"No escalated tickets.":"Select a ticket to begin."}</p>
          </div>
        )}
      </div>
    </div>
