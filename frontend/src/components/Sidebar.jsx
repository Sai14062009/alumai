...
import { motion, AnimatePresence } from 'framer-motion';
import { BarChart2, GitBranch, AlertTriangle, Activity, Gauge, Sun, Moon, LogOut, ChevronLeft, ChevronRight } from 'lucide-react';
import { useTheme, useSidebar, useAuth } from '../App';
import clsx from 'clsx';
...
export default function Sidebar({ currentPage, setPage, escalatedCount }) {
  ...
  const navItems = [
    ...
    { id: 'escalation', label: 'Escalation Hub', icon: AlertTriangle, badge: escalatedCount },
    { id: 'monitor', label: 'Asset Monitor', icon: Activity },
    { id: 'operations', label: 'Plant Operations', icon: Gauge },
  ];
  ...
  return (
    ...
    >
      {/* Header */}
      <div className={clsx("flex items-center pt-6 pb-4 relative", collapsed ? "px-0 justify-center" : "px-5 justify-between")}>
        <div className={clsx("flex items-center gap-3 cursor-pointer group", collapsed && "justify-center")} onClick={() => setPage('landing')}>
          <div className="relative flex-shrink-0">
            <div className="w-10 h-10 rounded-2xl flex items-center justify-center relative overflow-hidden"
              style={{ background: 'linear-gradient(135deg, rgba(0,229,155,0.15), rgba(0,212,255,0.08))' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
          ...
          <AnimatePresence>
            {!collapsed && (
              <motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} transition={{ duration: 0.2 }} className="overflow-hidden whitespace-nowrap">
                <h1 className="font-display font-bold text-[16px] tracking-tight leading-tight" style={{ color: 'var(--text-primary)' }}>Alum<span className="gradient-text">AI</span></h1>
                <p className="font-mono text-[8px] tracking-[0.2em] uppercase" style={{ color: 'var(--text-muted)' }}>Intelligence</p>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <AnimatePresence>
          {!collapsed && (
            <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} whileTap={{ scale: 0.85 }}
              onClick={() => setCollapsed(true)} className="w-7 h-7 rounded-lg flex items-center justify-center transition-colors haptic"
              style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', color: 'var(--text-muted)' }}>
              <ChevronLeft className="w-3.5 h-3.5" />
            </motion.button>
          )}
      ...
      {collapsed && (
        <div className="flex justify-center pb-2">
          <motion.button whileTap={{ scale: 0.85 }} onClick={() => setCollapsed(false)}
            className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors haptic"
            style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', color: 'var(--text-muted)' }}>
            <ChevronRight className="w-3.5 h-3.5" />
          </motion.button>
      ...
      <nav className={clsx("flex-1 space-y-1 overflow-y-auto", collapsed ? "px-2" : "px-3")}>
        {navItems.map((item) => {
          ...
          return (
            <div key={item.id} className="relative group">
              <motion.button whileTap={{ scale: 0.95 }} onClick={() => setPage(item.id)}
                className={clsx("w-full flex items-center rounded-2xl font-sans text-[13px] font-medium transition-all duration-200 relative",
                  collapsed ? "justify-center px-0 py-3" : "gap-3 px-3.5 py-3")}
                style={{ background: isActive ? 'var(--gradient-sidebar-active)' : 'transparent', color: isActive ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
                {isActive && (
                  <motion.div layoutId="nav-active-bar" className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-6 rounded-r-full"
                    style={{ background: 'linear-gradient(180deg, #00E59B, #00D4FF)', boxShadow: '0 0 12px rgba(0,229,155,0.4)' }}
                    transition={{ type: "spring", stiffness: 400, damping: 30 }} />
                )}
                <div className={clsx("flex-shrink-0 rounded-xl flex items-center justify-center transition-all duration-200", collapsed ? "w-10 h-10" : "w-9 h-9")}
                  style={{ background: isActive ? 'rgba(0,229,155,0.12)' : 'var(--bg-surface)', border: `1px solid ${isActive ? 'rgba(0,229,155,0.15)' : 'var(--border)'}`, boxShadow: isActive ? '0 0 16px rgba(0,229,155,0.08)' : 'none' }}>
                  <Icon className="w-[16px] h-[16px]" style={{ color: isActive ? '#00E59B' : 'var(--text-muted)' }} />
                </div>
                <AnimatePresence>
                  {!collapsed && (
                    <motion.span initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} transition={{ duration: 0.2 }}
                      className="flex-1 text-left overflow-hidden whitespace-nowrap font-semibold text-[13px]">{item.label}</motion.span>
                  )}
                </AnimatePresence>
                {item.badge > 0 && (
                  <span className={clsx("flex items-center justify-center rounded-full text-[9px] font-bold",
                    collapsed ? "absolute -top-0.5 -right-0.5 w-4 h-4" : "min-w-[22px] h-[22px] px-1.5")}
                    style={{ background: 'linear-gradient(135deg, #FF4D6A, #FF6B8A)', color: '#fff', boxShadow: '0 0 10px rgba(255,77,106,0.3)' }}>
                    {collapsed ? '' : item.badge}
                    {collapsed && <span className="w-1.5 h-1.5 rounded-full bg-white" />}
      ...
      <div className={clsx("pb-4 space-y-1.5", collapsed ? "px-2" : "px-3")}>
        ...
        <div className="relative group">
          <motion.button whileTap={{ scale: 0.9 }} onClick={toggleTheme}
            className={clsx("w-full flex items-center rounded-2xl font-sans text-[12px] font-medium transition-all duration-200",
              collapsed ? "justify-center px-0 py-2.5" : "gap-3 px-3.5 py-2.5")}
            style={{ color: 'var(--text-secondary)' }}>
            <div className={clsx("flex-shrink-0 rounded-xl flex items-center justify-center", collapsed ? "w-10 h-10" : "w-9 h-9")}
              style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
            ...
            <AnimatePresence>
              {!collapsed && (<motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="overflow-hidden whitespace-nowrap">{theme === 'dark' ? 'Light Mode' : 'Dark Mode'}</motion.span>)}
            </AnimatePresence>
          </motion.button>
          {collapsed && (
            <div className="absolute left-full top-1/2 -translate-y-1/2 ml-3 px-3 py-2 rounded-xl opacity-0 group-hover:opacity-100 pointer-events-none transition-all duration-200 whitespace-nowrap z-50"
        ...
        <div className="relative group">
          <motion.button whileTap={{ scale: 0.9 }} onClick={handleLogout}
            ...
            <AnimatePresence>
              {!collapsed && (<motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="overflow-hidden whitespace-nowrap">Sign Out</motion.span>)}
            </AnimatePresence>
          </motion.button>
          {collapsed && (
            <div className="absolute left-full top-1/2 -translate-y-1/2 ml-3 px-3 py-2 rounded-xl opacity-0 group-hover:opacity-100 pointer-events-none transition-all duration-200 whitespace-nowrap z-50"
        ...
        <div className={clsx("rounded-2xl flex items-center transition-all duration-300", collapsed ? "justify-center p-2" : "gap-3 p-3")}
          style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
          <div className={clsx("rounded-xl flex items-center justify-center font-display font-bold relative overflow-hidden flex-shrink-0",
            collapsed ? "w-10 h-10 text-[12px]" : "w-9 h-9 text-[11px]")}
            style={{ background: 'linear-gradient(135deg, rgba(0,229,155,0.2), rgba(0,212,255,0.15))', color: '#00E59B' }}>
            {displayName.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase()}
            <div className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2" style={{ background: '#00E59B', borderColor: 'var(--bg-surface)' }} />
          ...
          <AnimatePresence>
            {!collapsed && (
              <motion.div initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} transition={{ duration: 0.2 }} className="flex-1 min-w-0 overflow-hidden">
                <p className="font-sans text-[12px] font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{displayName}</p>
                <p className="font-mono text-[9px] truncate" style={{ color: 'var(--text-muted)' }}>{displayEmail}</p>
              </motion.div>
            )}
